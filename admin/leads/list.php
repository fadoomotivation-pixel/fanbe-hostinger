...
define('PAGE_TITLE', 'All Leads');

$statusFilter = $_GET['status'] ?? '';
$sourceFilter = $_GET['source'] ?? '';
$search = trim($_GET['q'] ?? '');

$sql = "SELECT l.*, u.full_name AS assigned_name, p.name AS project_name, c.full_name AS created_name
        FROM leads l
...
if ($sourceFilter) { $sql .= " AND l.source = ?"; $params[] = $sourceFilter; }
if ($search) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['assign'])) {
    ...
    $pdo->prepare("UPDATE leads SET assigned_to = ?, updated_at = NOW() WHERE id = ?")->execute([$empId, $leadId]);
    flashMessage('success', 'Lead assigned');
    redirect('/admin/leads/list.php?' . http_build_query($_GET));
}
...
<div class="filter-bar">
  <form method="GET" style="display:flex;gap:8px;flex-wrap:wrap;width:100%;">
    ...
    <select name="source" class="form-control" onchange="this.form.submit()" style="width:auto;">
      <option value="">All Sources</option>
      <?php foreach (['Website','Walk-in','Reference','Call','Social Media','MagicBricks','Google Ads'] as $s): ?>
        <option value="<?= $s ?>" <?= $sourceFilter === $s ? 'selected' : '' ?>><?= $s ?></option>
      <?php endforeach; ?>
    </select>
...
<div class="card">
  <div class="table-wrap">
    <table>
      <thead>
        <tr><th>Lead</th><th>Status</th><th>Source</th><th>Project</th><th>Assigned</th><th>Created</th><th>Actions</th></tr>
      </thead>
      <tbody>
        <?php if (empty($leads)): ?>
        ...
        <tr>
          ...
          <td>
            <form method="POST" style="display:flex;gap:4px;align-items:center;">
              ...
              <select name="employee_id" class="form-control" style="width:auto;padding:4px 8px;font-size:0.75rem;">
                <option value="">Unassigned</option>
                <?php foreach ($employees as $e): ?>
                  <option value="<?= $e['id'] ?>" <?= $lead['assigned_to'] == $e['id'] ? 'selected' : '' ?>>
                    <?= sanitize($e['full_name']) ?>
                  </option>
                <?php endforeach; ?>
              </select>
          ...
          <td>
            <a href="/employee/leads/view.php?id=<?= $lead['id'] ?>" class="btn btn-outline btn-sm">View</a>
          </td>
        </tr>
        <?php endforeach; ?>
      </tbody>
