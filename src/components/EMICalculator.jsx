...
import { Calculator, Info, TrendingUp, X, ChevronRight, IndianRupee } from 'lucide-react';
...
const BOOKING_PCT   = 0.30; // 30% booking amount
const EMI_MONTHS    = 12;   // 12 interest-free months
const ROI_PCT       = 0.15; // 15% annual appreciation estimate

// ── Breakdown Modal ────────────────────────────────────────────────────────
const BreakdownModal = ({ isOpen, onClose, data }) => {
  if (!data) return null;
  const { totalCost, booking, remaining, emi, project, sqyd } = data;
      ...
      <DialogContent className="max-w-md flex flex-col" style={{ maxHeight: '90vh' }}>
        ...
        <div className="flex-1 overflow-y-auto space-y-4 py-2">
          ...
          <div className="space-y-3">
            ...
            <div className="relative">
              ...
              <div className="space-y-2 pl-10">
                <div className="flex justify-between items-center p-3 bg-amber-50 border border-amber-100 rounded-lg">
                  <div>
                    <p className="text-sm font-semibold text-amber-800">Booking Amount (30%)</p>
                ...
                <div className="flex justify-between items-center p-3 bg-green-50 border border-green-100 rounded-lg">
                  <div>
                    <p className="text-sm font-semibold text-green-800">Remaining (70%)</p>
                    <p className="text-xs text-green-600">Split into {EMI_MONTHS} interest-free EMIs</p>
                  </div>
            ...
            <div className="flex justify-between items-center p-4 bg-[#0F3A5F] text-white rounded-xl">
              <div>
                <p className="text-xs text-blue-200">Monthly EMI (for {EMI_MONTHS} months)</p>
              ...
              {[1, 3, 5].map(yr => (
                <div key={yr} className="bg-white rounded-lg p-2 border border-purple-100">
                  ...
                  <p className="text-sm font-bold text-purple-700">
                    {fmtL(totalCost * Math.pow(1 + ROI_PCT, yr))}
                  </p>
                </div>
              ))}
            </div>
          ...
          <div className="space-y-2">
            {[
              'Registry starts after 30% booking payment',
        ...
        <div className="shrink-0 pt-3 border-t">
          <Button
            ...
            onClick={() => {
              onClose();
              window.open(`https://wa.me/919999999999?text=Hi%2C%20I%27m%20interested%20in%20a%20${sqyd}%20sq%20yd%20plot.%20Total%3A%20%E2%82%B9${fmt(totalCost)}`, '_blank');
            }}
          >
...
const EMICalculator = ({ defaultProjectId }) => {
  ...
  const [sqyd, setSqyd] = useState(100);
  ...
  const calc = useMemo(() => {
    ...
    const booking   = Math.round(totalCost * BOOKING_PCT);
    const remaining = totalCost - booking;
    const emi       = Math.round(remaining / EMI_MONTHS);
      ...
      <div className="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
        ...
        <div className="p-6 space-y-6">
              ...
              {projects.map(p => (
                ...
                >
                  ...
                  <div className={`text-[10px] mt-0.5 ${selectedProjectId === p.id ? 'text-blue-200' : 'text-gray-400'}`}>
                    ₹{fmt(p.pricing.pricePerSqYdNum)}/sq yd
                  </div>
                </button>
              ))}
            </div>
          ...
          <div className="space-y-2">
            ...
            <div className="relative pt-1">
              <input
                ...
                style={{
                  background: `linear-gradient(to right, #0F3A5F ${sliderFill}%, #e5e7eb ${sliderFill}%)`
                }}
              />
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-[#0F3A5F] rounded-xl p-4 text-white col-span-2">
              ...
              <p className="text-xs text-blue-300 mt-1">
                {sqyd} sq yd × ₹{fmt(rate)}/sq yd
              </p>
            </div>
            <div className="bg-amber-50 border border-amber-100 rounded-xl p-4">
              <p className="text-xs text-amber-600 font-medium mb-1">Booking (30%)</p>
            ...
            <div className="bg-green-50 border border-green-100 rounded-xl p-4">
              ...
              <p className="text-[10px] text-green-500 mt-1">{EMI_MONTHS} months · 0% interest</p>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between bg-gradient-to-r from-purple-50 to-indigo-50 border border-purple-100 rounded-xl px-4 py-3">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-sm font-bold text-purple-700">
                  {fmtL(calc.totalCost * Math.pow(1 + ROI_PCT, 3))}
                </p>
              </div>
            </div>
          ...
          <div className="flex gap-3">
            <Button
              variant="outline"
              className="flex-1 gap-2 border-[#0F3A5F] text-[#0F3A5F] hover:bg-blue-50"
              onClick={() => setShowBreakdown(true)}
            >
              <Info size={15} /> Full Breakdown
            </Button>
            <Button
              className="flex-1 gap-2 bg-amber-500 hover:bg-amber-600 text-white font-bold"
              onClick={() => window.open(`https://wa.me/919999999999?text=Hi%2C%20I%27m%20interested%20in%20${sqyd}%20sq%20yd%20plot%20at%20${encodeURIComponent(selectedProject.name)}.%20Total%3A%20%E2%82%B9${fmt(calc.totalCost)}`, '_blank')}
            >
              Book Now
            </Button>
          </div>
