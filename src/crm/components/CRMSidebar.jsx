...
import {
  ...
  User, CheckSquare, CalendarCheck
} from 'lucide-react';
...
const IconMap = {
  ...
  Phone, PhoneCall, TrendingUp, PieChart, User, CheckSquare, CalendarCheck
};
...
const CRMSidebar = ({ isOpen, onClose }) => {
  ...
  if (user && user.role !== ROLES.SUPER_ADMIN) {
    ...
    if (!hasPromo) {
      rawItems.push({ label: 'Promo Materials', path: '/crm/sales/tools', icon: 'FileText' });
    }
  }
  ...
  // Build grouped menu: inject section headers before first HR item
  const renderMenu = () => {
    let lastGroup = null;
    return rawItems.map((item, idx) => {
      const Icon = IconMap[item.icon] || LayoutDashboard;
      const groupHeader = item.group && item.group !== lastGroup ? (
        <div key={`group-${item.group}`} className="px-4 pt-4 pb-1">
    ...
    <>
      ...
      `}>
        ...
        <nav className="flex-1 overflow-y-auto py-4 px-3 space-y-1">
          ...
          {/* Super Admin Promo Materials shortcut */}
