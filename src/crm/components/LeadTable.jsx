...
const LeadTable = ({
  ...
  // ✅ Quick-select N: select first N from the SORTED list (not yet selected ones merged)
  const handleSelectN = (n) => {
    const topN = sortedLeads.slice(0, n).map(l => l.id);
    // merge with already selected from other pages / lists
    const merged = [...new Set([...selectedIds, ...topN])];
    // call onSelectLead for each newly added id
    topN.forEach(id => {
      if (!selectedIds.includes(id)) onSelectLead(id, true);
    });
  };
  ...
  // Quick-select sizes to show (only show if there are enough leads)
  const quickSelectSizes = [10, 25, 50].filter(n => n < leads.length);

  return (
    <div className="space-y-3">

      {/* ── QUICK SELECT TOOLBAR (always visible when > 0 leads) ── */}
      {leads.length > 0 && (
          ...
          return (
            <div
              ...
              className={`bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex flex-col gap-3 ${
                selectedIds.includes(lead.id) ? 'ring-2 ring-blue-500 bg-blue-50' : ''
              }`}
            >
