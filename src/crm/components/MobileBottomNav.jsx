...
// 4 tabs: Call CRM (primary) | Follow Up | Leads | Profile
...
const NAV_ITEMS = [
  ...
  { id: 'followup', label: 'Follow Ups', icon: Clock,  path: '/crm/sales/crm?tab=follow_up', activeColor: 'text-yellow-600', activeBg: 'bg-yellow-50' },
          ...
          {NAV_ITEMS.map((item) => (
            ...
            >
              {({ isActive }) => (
                <>
                  {/* Active top bar */}
                  {isActive && <div className="absolute top-0 inset-x-0 h-0.5 bg-current rounded-b-full" />}
