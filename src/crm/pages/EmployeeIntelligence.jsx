...
import {
  ...
  Shuffle, PieChart, ArrowUp, ArrowDown, Minus, Trophy
} from 'lucide-react';
...
import {
  ...
  suggestLeadRedistribution,
  analyzeWinLossInsights
...
const EmployeeIntelligence = () => {
  ...
  const performanceTrends = useMemo(() =>
    calculatePerformanceTrends(employees, calls, siteVisits, bookings),
    [employees, calls, siteVisits, bookings]
  );

  // ═══ NEW: Lead Redistribution Suggestions ═══
  const redistributionSuggestions = useMemo(() =>
    suggestLeadRedistribution(employees, leads, calls),
    [employees, leads, calls]
  );

  // ═══ NEW: Win/Loss Insights ═══
  const winLossInsights = useMemo(() =>
    analyzeWinLossInsights(leads, calls, bookings),
    ...
    <div className="space-y-6 pb-20">
      ...
      <Tabs value={period} onValueChange={setPeriod}>
        ...
        <TabsContent value={period} className="mt-4 space-y-6">
            ...
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="flex items-center gap-2 text-sm">
                  ...
                  Performance Trends (This Week vs Last Week)
                </CardTitle>
              </CardHeader>
              <CardContent>
                <div className="space-y-3">
                  {performanceTrends.map((emp) => (
                    <div key={emp.employeeId} className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg border">
                      <div className="w-24 truncate">
                        <p className="text-xs font-bold text-gray-800">{emp.employeeName}</p>
                      </div>
                      <div className="flex-1 grid grid-cols-4 gap-2">
                        {[
                          { label: 'Calls', data: emp.calls },
                          { label: 'Visits', data: emp.visits },
                          { label: 'Bookings', data: emp.bookings },
                          { label: 'Overall', data: emp.overallTrend, isOverall: true },
                        ].map((metric, i) => (
                          <div key={i} className="text-center">
                            <div className="flex items-center justify-center gap-1">
                              {metric.isOverall ? (
                                <span className={`text-sm font-bold ${
                                  metric.data.direction === 'up' ? 'text-green-600' :
                                  metric.data.direction === 'down' ? 'text-red-600' : 'text-gray-500'
                                }`}>
                                  {metric.data.direction === 'up' ? <ArrowUp size={14} /> :
                                   metric.data.direction === 'down' ? <ArrowDown size={14} /> :
                                   <Minus size={14} />}
                                </span>
                              ) : (
                                <>
                                  <span className="text-sm font-bold text-gray-800">{metric.data.current}</span>
                                  <span className={`text-[10px] flex items-center ${
                                    metric.data.trend.direction === 'up' ? 'text-green-600' :
                                    metric.data.trend.direction === 'down' ? 'text-red-600' : 'text-gray-400'
                                  }`}>
                                    {metric.data.trend.direction === 'up' ? <ArrowUp size={10} /> :
                                     metric.data.trend.direction === 'down' ? <ArrowDown size={10} /> :
                                     <Minus size={10} />}
                            ...
                            <p className="text-[9px] text-gray-400">{metric.label}</p>
                          </div>
                        ))}
                      </div>
                    </div>
          ...
          {/* ═══ SMART LEAD REDISTRIBUTION (NEW) ═══ */}
          {redistributionSuggestions.length > 0 && (
            <Card className="border-2 border-amber-200 bg-gradient-to-r from-amber-50 to-orange-50">
              <CardHeader className="pb-2">
                <CardTitle className="flex items-center gap-2 text-sm">
                  <Shuffle className="h-4 w-4 text-amber-600" />
                  Smart Lead Redistribution
                  <Badge className="bg-amber-600 text-white text-[10px] ml-auto">{redistributionSuggestions.length} suggestions</Badge>
                </CardTitle>
              </CardHeader>
              <CardContent>
                <p className="text-xs text-amber-700 mb-3">These leads are neglected and could be reassigned to active employees for better results.</p>
                <div className="space-y-2">
                  {redistributionSuggestions.map((s, i) => (
                    <div key={i} className="bg-white p-3 rounded-lg border border-amber-200">
                      <div className="flex items-start justify-between mb-2">
                        <div>
                          <p className="text-sm font-bold text-gray-900">{s.leadName}</p>
                          <Badge className={`text-[10px] ${
                            s.interest === 'Hot' ? 'bg-red-100 text-red-700' :
                            s.interest === 'Warm' ? 'bg-yellow-100 text-yellow-700' :
                        ...
                        <Badge variant="outline" className="text-[10px] text-red-600 border-red-300">
                          {s.daysSinceContact}d no contact
                        </Badge>
                      </div>
                      <div className="flex items-center gap-2 text-xs text-gray-600">
                        <span className="text-red-600 font-medium">{s.currentOwner}</span>
                        <ArrowUpRight size={12} className="text-amber-500" />
                        <span className="text-green-600 font-medium">{s.suggestedOwner}</span>
                      </div>
          ...
          {/* ═══ WIN/LOSS INSIGHTS (NEW) ═══ */}
          {(winLossInsights.wins.count > 0 || winLossInsights.losses.count > 0) && (
              ...
              <CardHeader className="pb-2">
                <CardTitle className="flex items-center gap-2 text-sm">
                  ...
                  <Badge className="bg-indigo-100 text-indigo-700 text-[10px] ml-auto">
                    {winLossInsights.conversionRate}% conversion
                  </Badge>
                </CardTitle>
              </CardHeader>
              <CardContent>
                <div className="grid grid-cols-2 gap-4 mb-4">
                  ...
                  <div className="bg-green-50 rounded-lg p-4 border border-green-200">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <h4 className="font-bold text-green-800 text-sm">Wins</h4>
                    </div>
                    <p className="text-3xl font-black text-green-600 mb-2">{winLossInsights.wins.count}</p>
                    <div className="space-y-1 text-xs text-green-700">
                      {winLossInsights.wins.avgCallsToBook > 0 && (
                        <p>Avg {winLossInsights.wins.avgCallsToBook} calls to close</p>
                      )}
                      {winLossInsights.wins.avgDaysToClose > 0 && (
                        <p>Avg {winLossInsights.wins.avgDaysToClose} days to close</p>
                      )}
                      {winLossInsights.topWinSource && (
                        <p>Best source: {winLossInsights.topWinSource.source}</p>
                      )}
                    </div>
                  </div>
                  {/* Losses */}
                  <div className="bg-red-50 rounded-lg p-4 border border-red-200">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <h4 className="font-bold text-red-800 text-sm">Losses</h4>
                    </div>
                    <p className="text-3xl font-black text-red-600 mb-2">{winLossInsights.losses.count}</p>
                    <div className="space-y-1 text-xs text-red-700">
                      {winLossInsights.losses.avgCallsBeforeLoss > 0 && (
                        <p>Avg {winLossInsights.losses.avgCallsBeforeLoss} calls before loss</p>
                      )}
                      {Object.entries(winLossInsights.losses.commonSources).slice(0, 2).map(([source, count]) => (
                        <p key={source}>From {source}: {count}</p>
                      ))}
                    </div>
                  </div>
                </div>
                {/* Key Insights */}
                {winLossInsights.insights.length > 0 && (
                  <div className="bg-indigo-50 rounded-lg p-3 border border-indigo-200">
                    <div className="flex items-center gap-2 mb-2">
                      <Lightbulb className="h-4 w-4 text-indigo-600" />
                      <h4 className="font-bold text-indigo-800 text-xs">Key Insights</h4>
                    </div>
                    <ul className="space-y-1">
                      {winLossInsights.insights.map((insight, i) => (
