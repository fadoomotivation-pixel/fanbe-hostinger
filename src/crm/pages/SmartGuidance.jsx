...
import {
  ...
  ChevronDown, ChevronUp, Crown, Award, Sunrise,
  Gift, Rocket, CheckCircle
} from 'lucide-react';
...
const SmartGuidance = () => {
  ...
  const prioritizedLeads = useMemo(() => {
    ...
    const leadsWithScores = myLeads.map(lead => {
      ...
      const nextFollowUp = suggestNextFollowUp(lead, calls);
  ...
  const briefing = useMemo(() => {
    if (!user?.uid) return null;
    return generateMorningBriefing(user.uid, leads, calls, siteVisits, bookings);
  }, [user, leads, calls, siteVisits, bookings]);

  // ── DAILY STREAK & GAMIFICATION ──
  const streak = useMemo(() => {
    if (!user?.uid) return null;
    return calculateStreak(user.uid, calls);
  }, [user, calls]);

  // ── QUICK WINS ──
  const quickWins = useMemo(() => {
    if (!myLeads.length || !calls) return [];
    return identifyQuickWins(myLeads, calls);
  }, [myLeads, calls]);
  ...
  return (
    <div className="space-y-6 pb-20">
      ...
      {/* ═══ MORNING BRIEFING ═══ */}
      {briefing && (
        <Card className="border-2 border-indigo-200 bg-gradient-to-r from-indigo-50 via-blue-50 to-purple-50 shadow-lg">
          <CardContent className="p-5">
            <div className="flex items-center gap-2 mb-3">
              <Sunrise className="h-6 w-6 text-indigo-600" />
              <h2 className="font-bold text-indigo-900 text-lg">{briefing.greeting}!</h2>
            </div>
            <div className="grid grid-cols-4 gap-3 mb-4">
              <div className="bg-white rounded-lg p-2 text-center border">
                <p className="text-xl font-black text-indigo-700">{briefing.totalLeads}</p>
                <p className="text-[10px] text-gray-500">Active Leads</p>
              </div>
              <div className="bg-white rounded-lg p-2 text-center border">
                <p className="text-xl font-black text-orange-600">{briefing.todayFollowUps}</p>
                <p className="text-[10px] text-gray-500">Today F/U</p>
              </div>
              <div className="bg-white rounded-lg p-2 text-center border">
                <p className="text-xl font-black text-red-600">{briefing.uncontactedHot}</p>
                <p className="text-[10px] text-gray-500">Hot Pending</p>
              </div>
              <div className="bg-white rounded-lg p-2 text-center border">
                <p className="text-xl font-black text-green-600">{briefing.newLeads}</p>
                <p className="text-[10px] text-gray-500">New Today</p>
              </div>
            </div>
            {briefing.yesterdayCalls > 0 && (
              <p className="text-xs text-gray-600 mb-3">
                Yesterday: {briefing.yesterdayCalls} calls, {briefing.yesterdayConnected} connected
              </p>
            )}
              ...
              {briefing.actions.map((action, i) => (
                ...
                >
                  {action.priority === 'critical' ? <AlertCircle size={14} /> :
                   action.priority === 'high' ? <Flame size={14} /> :
                   action.priority === 'medium' ? <Target size={14} /> :
                   <ThumbsUp size={14} />}
                  {action.text}
                </div>
              ))}
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-bold text-amber-900">Daily Streak</h3>
              </div>
              <Badge className={`text-sm font-bold ${
                streak.currentStreak >= 7 ? 'bg-purple-600 text-white' :
                streak.currentStreak >= 3 ? 'bg-orange-500 text-white' :
            ...
            <div className="grid grid-cols-3 gap-3 mb-3">
              <div className="text-center bg-white rounded-lg p-2 border">
                <p className="text-lg font-black text-orange-600">{streak.currentStreak}</p>
                <p className="text-[10px] text-gray-500">Current</p>
              </div>
              <div className="text-center bg-white rounded-lg p-2 border">
                <p className="text-lg font-black text-purple-600">{streak.bestStreak}</p>
                <p className="text-[10px] text-gray-500">Best Streak</p>
              </div>
              <div className="text-center bg-white rounded-lg p-2 border">
                <p className="text-lg font-black text-blue-600">{streak.totalCalls}</p>
              ...
              <div className="flex flex-wrap gap-2">
                {streak.badges.map((badge, i) => (
                  <Badge
                    ...
                    variant="outline"
                    className={`text-xs ${
                      badge.color === 'purple' ? 'border-purple-400 text-purple-700 bg-purple-50' :
                      badge.color === 'orange' ? 'border-orange-400 text-orange-700 bg-orange-50' :
                      badge.color === 'blue' ? 'border-blue-400 text-blue-700 bg-blue-50' :
                      badge.color === 'gold' ? 'border-yellow-400 text-yellow-700 bg-yellow-50' :
                      badge.color === 'green' ? 'border-green-400 text-green-700 bg-green-50' :
                      'border-amber-400 text-amber-700 bg-amber-50'
                    }`}
                  >
                    {badge.color === 'purple' ? <Shield size={12} className="mr-1" /> :
                     badge.color === 'orange' ? <Flame size={12} className="mr-1" /> :
                     badge.color === 'gold' ? <Crown size={12} className="mr-1" /> :
                     badge.color === 'green' ? <Zap size={12} className="mr-1" /> :
      ...
      {/* ═══ QUICK WINS - LEADS CLOSEST TO CONVERSION ═══ */}
      {quickWins.length > 0 && (
        <Card className="border-2 border-green-200 bg-gradient-to-r from-green-50 to-emerald-50 shadow-md">
          <CardContent className="p-5">
            <div className="flex items-center gap-2 mb-3">
              <Rocket className="h-5 w-5 text-green-600" />
              <h2 className="font-bold text-green-900">Quick Wins - Close These First!</h2>
              <Badge className="bg-green-600 text-white text-xs ml-auto">{quickWins.length}</Badge>
            </div>
            <p className="text-xs text-green-700 mb-3">These leads have the highest chance of converting based on interest, budget, and engagement.</p>
            <div className="space-y-2">
              {quickWins.map((lead, i) => (
                <div
                  key={lead.id}
                  className="flex items-center gap-3 bg-white p-3 rounded-lg border border-green-200 cursor-pointer hover:shadow-md transition"
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap gap-1 mt-1">
                      {lead.winReasons.map((reason, j) => (
                        <Badge key={j} variant="outline" className="text-[10px] border-green-300 text-green-700">
                          {reason}
                        </Badge>
                      ))}
                    </div>
                  ...
                  <div className="flex flex-col items-end gap-1 shrink-0">
                    <Badge className="bg-green-600 text-white text-xs">{lead.winScore}%</Badge>
                    <Button
                      size="sm"
                      className="bg-green-600 hover:bg-green-700 h-8 px-3 text-xs"
                      onClick={(e) => {
                        e.stopPropagation();
                        initiateCallWithQuickLog(lead);
                      }}
                    >
                ...
                <CardContent className="p-4">
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      ...
                      {lead.nextFollowUp && (
                        <div className={`flex items-center gap-2 text-xs p-1.5 rounded mb-2 ${
                          lead.nextFollowUp.urgency === 'critical' ? 'bg-red-50 text-red-700' :
                          lead.nextFollowUp.urgency === 'high' ? 'bg-orange-50 text-orange-700' :
                          'bg-blue-50 text-blue-700'
                        }`}>
                          <Calendar size={12} />
                          <span className="font-medium">{lead.nextFollowUp.reason}</span>
                        </div>
                      )}
