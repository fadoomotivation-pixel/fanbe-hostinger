...
// Reads employees from CRM 'employees' table.
// Employees (any role) can self-mark their own attendance via SelfAttendanceWidget.
...
// ── Self-Attendance Widget (shown to every employee role) ─────────────────────
const SelfAttendanceWidget = ({ user, employees, onMarked }) => {
  const { toast } = useToast();
  const [saving,   setSaving]   = useState(false);
  const [todayRec, setTodayRec] = useState(null); // existing record for today
  ...
  // Find emp_id by matching CRM user id
  const myEmpId = user?.id;
  ...
  const markAttendance = async (status) => {
    ...
    const { error } = await supabaseAdmin.from('hr_attendance').upsert(payload, { onConflict: 'emp_id,att_date' });
    setSaving(false);
              ...
              <div className="flex items-center justify-between flex-wrap gap-3">
                <div>
                  ...
                  <p className="text-xs mt-0.5">
                    ...
                    {todayRec.out_time && <> &nbsp;&bull;&nbsp; Out: {fmtTime(todayRec.out_time)}</>}
                    {todayRec.work_hours && <> &nbsp;&bull;&nbsp; <strong>{todayRec.work_hours}h worked</strong></>}
                  </p>
                </div>
                <div className="flex gap-2">
                  ...
                  <Button size="sm" variant="outline" onClick={() => setShowLeave(true)} className="border-blue-400 text-blue-600 hover:bg-blue-50">
      ...
      <Dialog open={showLeave} onOpenChange={setShowLeave}>
        <DialogContent className="max-w-md">
          ...
          <form onSubmit={handleLeaveSubmit} className="space-y-4">
            ...
            <div className="space-y-1"><Label>Reason</Label><Input value={leaveForm.reason} onChange={e => setLeaveForm(p=>({...p,reason:e.target.value}))} placeholder="Brief reason for leave" /></div>
            <DialogFooter>
...
const HRAttendance = () => {
  ...
  // ── Load from CRM 'employees' table ──────────────────────────────────────
  const load = async () => {
    setLoading(true);
    const [empRes, attRes, leaveRes, holRes] = await Promise.all([
      supabaseAdmin.from('employees').select('id,name,email,role').order('name'),
  ...
  return (
    <div className="space-y-6 pb-10">
      ...
      {/* ✅ Self-Attendance Widget — visible to ALL roles */}
      <SelfAttendanceWidget user={user} employees={employees} onMarked={load} />

      {/* My Attendance Summary (non-admin) */}
      {!isAdmin && (
        <Card>
          <CardHeader className="pb-2">
